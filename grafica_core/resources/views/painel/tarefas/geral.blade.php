<x-layouts.app titulo="Quadro Geral - Gráfica Vapt Vupt">
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black text-brand-secondary tracking-tight">Quadro Geral (Kanban)</h1>
            <p class="text-slate-500 mt-1 font-medium">Visão corporativa de todas as equipes</p>
        </div>
        <button onclick="document.getElementById('modalNovaTarefaGeral').classList.remove('hidden')" class="btn btn-primary rounded-xl px-5 py-2.5 font-bold shadow-md hover:-translate-y-1 transition-all bg-brand-primary text-white">
            Nova Tarefa
        </button>
    </div>
                ...
                <div class="flex-1 overflow-y-auto px-4 pb-4 space-y-4">
                    @forelse($tarefas[$k] as $t)
                        <div class="bg-white rounded-xl p-4 shadow-sm border border-slate-200 hover:shadow-md transition-shadow relative group">
                            <form action="{{ route('admin.ops.tasks.destroy', $t->id) }}" method="POST" class="absolute top-3 right-3 hidden group-hover:block" onsubmit="return confirm('Excluir definitivamente esta tarefa do Kanban corporativo?');">
                                @csrf @method('DELETE')
                                <button type="submit" class="text-red-500 hover:text-red-700 bg-red-50 rounded-lg p-1 z-10 relative" title="Excluir"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                            </form>
                            ...
                            <h4 class="font-bold text-slate-800 text-sm leading-snug pr-6">{{ $t->titulo }}</h4>
                            @if($t->descricao)
                            ...
                            <div class="mt-4 pt-3 border-t border-slate-100 flex justify-between items-center text-xs">
                                <span class="text-brand-primary font-bold">Resp: {{ $t->responsavel->nome ?? 'Sem dono' }}</span>
                                @if($t->prazo)
                            ...
                            <div class="mt-3 flex flex-wrap gap-1">
                                <form action="{{ route('admin.ops.tasks.status', $t->id) }}" method="POST">
                                    @csrf @method('PATCH')
                                    <select name="status" onchange="this.form.submit()" class="text-xs border-slate-200 rounded-lg bg-slate-50 py-1 pl-2 pr-6 focus:border-brand-primary focus:ring-brand-primary w-full">
    ...
    <!-- Modal Nova Tarefa Geral -->
    <div id="modalNovaTarefaGeral" class="fixed inset-0 z-[100] hidden">
        <div class="absolute inset-0 bg-slate-900/50 backdrop-blur-sm" onclick="document.getElementById('modalNovaTarefaGeral').classList.add('hidden')"></div>
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white rounded-2xl shadow-2xl p-6">
            <h2 class="text-2xl font-bold text-brand-secondary mb-4">Criar Tarefa Corporativa</h2>
            <form action="{{ route('admin.ops.tasks.store') }}" method="POST">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="col-span-2">
                            <label class="block text-sm font-bold text-slate-700 mb-1">Atribuir a</label>
                            <select name="responsavel_id" class="w-full rounded-xl border-slate-200 focus:border-brand-primary focus:ring-brand-primary">
                                <option value="">Sem dono (Pendente de atribuição)</option>
                                @foreach($usuarios as $u)
                                    <option value="{{ $u->id }}">{{ $u->nome }} ({{ ucfirst($u->perfil) }})</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1">Prazo</label>
                ...
                <div class="mt-6 flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modalNovaTarefaGeral').classList.add('hidden')" class="px-5 py-2 font-bold text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">Cancelar</button>
