...
<nav {{ $attributes->merge(['class' => $mobile ? 'space-y-5' : 'space-y-6']) }}>
    @php $activeItemLocked = false; @endphp
    @foreach($sections as $section)
        <div>
            @if(!empty($section['section']))
                <div class="{{ $mobile ? 'px-2 mb-1.5' : 'px-3 mb-2' }}">
                    <p class="text-[10px] font-black uppercase tracking-wider {{ $mobile ? 'text-slate-400' : 'text-slate-400/60 transition-colors group-hover:text-slate-400' }}">
                        {{ $section['section'] }}
                    </p>
                </div>
            @endif
            ...
            <ul class="space-y-1">
                @foreach($section['items'] as $item)
                    @php
                        if ($hidePdv && (($item['is_pdv'] ?? false) === true)) {
                            continue;
                        }

                        $itemMatchesCurrentRoute = \App\Support\Menu\MenuState::isActive($item);
                        $isActive = !$activeItemLocked && $itemMatchesCurrentRoute;
                        if ($isActive) {
                    ...
                    <li>
                        <a href="{{ $href }}"
                           ...
                           @if($closeOnClick) @click="mobileMenuOpen = false" @endif
                           class="sidebar-menu-item group {{ $colorClass }} {{ $mobile ? '' : ($isActive ? 'translate-x-1' : 'hover:translate-x-1') }}">
                            
                            <div class="relative flex h-5 w-5 shrink-0 items-center justify-center">
                                <x-icon :name="$item['icon']" class="h-5 w-5 shrink-0 transition-transform duration-300 group-hover:scale-110" />
